...
export function BookingButton({ item, large = false }: { item: PublicGroupClass; large?: boolean }) {
  ...
  const booked = item.isBookedByCurrentClient;
  const full = item.isFullyBooked || item.availableSeats <= 0;
  return <Button size={large ? "lg" : "sm"} className="min-h-11 w-full rounded-full sm:w-auto" variant={booked || full || started ? "secondary" : "primary"} disabled={!authReady || started || (!booked && full)} onClick={() => act({ type: booked ? "cancel" : "book", item })}>
...
export function GroupClassCard({ item }: { item: PublicGroupClass }) {
  return <article className="overflow-hidden rounded-[var(--radius-xl)] bg-surface-container-low">
    <div className="p-4 sm:p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><Link className="break-words font-display text-lg font-bold hover:text-primary-light sm:text-2xl" href={`/classes/${encodeURIComponent(item.publicSlug)}`}>{item.title}</Link><AvailabilityBadge item={item} /></div><p className="mt-2 flex flex-wrap items-center gap-1.5 text-xs sm:text-sm"><Clock3 size={13} className="text-primary-light" />{studioTime(item.startAt)}–{studioTime(item.endAt)} <span className="text-on-surface-variant">({durationMinutes(item.startAt, item.endAt)} min) • {studioDay(item.startAt)}</span></p></div><div className="hidden sm:block"><BookingButton item={item} /></div></div>{item.note && <p className="mt-4 hidden text-sm leading-6 text-on-surface-variant sm:block">{item.note}</p>}</div>
...
export function PackageList({ items, layout = "stack" }: { items: PublicGroupPackage[]; layout?: "stack" | "grid" }) {
  const { act, authReady } = usePublicContext();
  return <section className="scroll-mt-6 rounded-[var(--radius-xl)] bg-surface-container-low p-4 sm:p-5"><div className="mb-5 flex items-center gap-3"><span className="rounded-[var(--radius-md)] bg-primary/15 p-3 text-primary-light"><Ticket size={20} /></span><div><h2 className="font-display text-lg font-bold">Pakiety zajęć grupowych</h2><p className="mt-1 text-xs text-on-surface-variant">{items[0] ? `Wejściówki • ${items[0].locationName}` : "Pakiety w wybranej lokalizacji"}</p></div></div>
    <div className={layout === "grid" ? "grid min-w-0 gap-4 md:grid-cols-2 lg:grid-cols-3" : "space-y-3"}>{items.map((item) => <article key={item.id} className="flex min-w-0 flex-col rounded-[var(--radius-lg)] bg-surface-container p-5"><div className="flex flex-wrap items-start justify-between gap-3"><h3 className="min-w-0 break-words text-sm font-semibold">{item.name}</h3><strong className="shrink-0 text-lg">{publicMoney(item.price, item.currency)}</strong></div><p className="mt-3 text-xs text-on-surface-variant">{item.entriesCount} wejść • {item.durationDays} dni</p><p className="mt-2 flex items-center gap-1 text-xs text-on-surface-variant"><MapPin size={13} className="shrink-0" />{item.locationName}</p>{item.description && <p className="mt-2 text-xs leading-5 text-on-surface-variant">{item.description}</p>}{item.entriesCount > 1 && <p className="mt-2 flex items-center gap-1 text-xs text-tertiary-light"><Check size={13} />{publicMoney(item.price / item.entriesCount, item.currency)} / wejście</p>}<div className="flex-1" /><Button disabled={!authReady} onClick={() => act({ type: "buy", item })} variant={item.entriesCount > 1 ? "primary" : "secondary"} className="mt-5 min-h-11 w-full rounded-full" size="sm">Kup pakiet</Button></article>)}{!items.length && <p className="p-3 text-sm text-on-surface-variant">Brak dostępnych pakietów w tej lokalizacji.</p>}</div>
