...
export default function GroupClassListing() {
  ...
  useEffect(() => {
    ...
    }).catch((err) => { if (active) { setError(publicErrorMessage(err)); setLoading(false); } });
    return () => { active = false; };
  }, [retry]);
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      Promise.allSettled([getPublicGroupClasses({ locationId, from: `${date}T00:00:00`, to: `${date}T23:59:59` }), getPublicPackages(locationId)]).then(([sessionsResult, packagesResult]) => {
        ...
        else { setClasses([]); setError(publicErrorMessage(sessionsResult.reason)); }
        if (packagesResult.status === "fulfilled") setPackages(packagesResult.value);
        else { setPackages([]); setPackageError(publicErrorMessage(packagesResult.reason)); }
        setLoading(false);
      });
    }, 0);
  ...
  return <>
    ...
    <section id="locations" tabIndex={-1} className="scroll-mt-6 focus:outline-2 focus:outline-primary-light/50 mb-8 flex flex-wrap items-center justify-between gap-5 rounded-[var(--radius-xl)] bg-surface-container-low p-4 sm:mb-9 sm:p-6"><div className="min-w-0 w-full sm:w-auto"><p className="mb-3 text-[10px] font-semibold uppercase tracking-wider text-on-surface-variant">Wybierz studio</p><div className="flex flex-wrap gap-1 rounded-[var(--radius-lg)] sm:bg-surface-container-lowest sm:p-1">{locations.map((item) => <Button key={item.id} size="sm" variant={item.id === locationId ? "primary" : "ghost"} aria-pressed={item.id === locationId} onClick={() => { if (item.id !== locationId) { setLoading(true); setLocationId(item.id); } }} className="min-h-11 rounded-full sm:rounded-[var(--radius-md)]">{item.id === locationId && <MapPin size={14} />}{item.name}</Button>)}</div></div>{location?.address && <p className="hidden max-w-xs text-sm text-on-surface-variant sm:block"><span className="block text-xs">Wybrana lokalizacja:</span><strong>{location.name}</strong> • {location.address}</p>}{locationsLoaded && !locations.length && <p className="text-sm text-on-surface-variant">Nie ma jeszcze lokalizacji z publicznymi zajęciami.</p>}</section>
    <div className="grid min-w-0 gap-7 lg:grid-cols-[minmax(0,1fr)_310px]">
      <section className="min-w-0"><h2 className="mb-4 font-display text-xl font-bold">Najbliższe zajęcia</h2><div className="mb-5 flex max-w-full gap-2 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">{today && Array.from({ length: 5 }, (_, index) => addStudioDays(today, index)).map((day, index) => <button key={day} onClick={() => selectDate(day)} aria-pressed={date === day} className={`min-h-14 min-w-20 shrink-0 rounded-[var(--radius-lg)] px-3 py-2 text-xs ${date === day ? "bg-primary text-on-primary" : "bg-surface-container-low text-on-surface-variant"}`}><span className="block text-[10px]">{index === 0 ? "Dzisiaj" : index === 1 ? "Jutro" : studioFormat(day, { weekday: "short" })}</span><span className="mt-1 block font-semibold">{studioFormat(day, { day: "numeric", month: "short" })}</span></button>)}</div>
        <label className="mb-5 block max-w-52"><span className="mb-2 block text-xs text-on-surface-variant">Inny termin</span><Input aria-label="Wybierz datę zajęć" type="date" value={date} onChange={(event) => selectDate(event.target.value)} /></label>
        ...
        <a href="#packages" className="mt-6 flex items-center gap-4 rounded-[var(--radius-xl)] bg-surface-container p-5"><Info className="shrink-0 text-primary-light" size={22} /><div className="flex-1"><h3 className="text-sm font-semibold">Nie masz jeszcze wejściówki?</h3><p className="mt-1 text-xs text-on-surface-variant">Do zapisu potrzebujesz opłaconego pakietu.</p></div><ArrowRight size={17} className="text-primary-light" /></a>
      </section>
      <aside id="packages" tabIndex={-1} className="min-w-0 scroll-mt-6 rounded-[var(--radius-xl)] focus:outline-2 focus:outline-primary-light/50">{loading ? <PublicLoading /> : packageError ? <PublicError message={packageError} retry={() => setRetry((value) => value + 1)} /> : <PackageList items={packages} />}</aside>
    </div>
  </>;
}
